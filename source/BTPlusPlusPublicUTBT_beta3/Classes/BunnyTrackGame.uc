...
function ScoreFlag(Pawn Scorer, CTFFlag theFlag)
{
	...
	if ( Scorer.PlayerReplicationInfo.Team == theFlag.Team || Scorer.Health < 1 || Scorer.AttitudeToPlayer == ATTITUDE_Frenzy)
		return;

	// if real CAP (nocp)
	if(Scorer.AttitudeToPlayer != ATTITUDE_Follow)
	{
	...
	{
		Controller.BTCaptureCP( Scorer );
		BroadcastMessage(Scorer.PlayerReplicationInfo.PlayerName $ " captured the flag with Checkpoints!", true, 'CriticalEvent');
	}
	...
	Scorer.AttitudeToPlayer = ATTITUDE_Frenzy;
