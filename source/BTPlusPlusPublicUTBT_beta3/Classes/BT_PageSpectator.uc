...
var		UMenuLabelControl			SpecTPLabel;
var		UMenuRaisedButton			SpecTPButton;
var 	UWindowSmallButton 			ResetSpecTPButton;
//-----------------------------------------------------------------------------
//	HotKey stuff
//-----------------------------------------------------------------------------
var 	bool 						bPolling;
...
function Created()
{
	...
	BehindviewAutoFollowCheck.SetText("Behindview auto view rotation");
	...
	BehindviewToggleLabel.SetText("Toggle auto view rotation on/off");
...
//	8	==========================================
	...
	SpecTPLabel = UMenuLabelControl(CreateControl(class'UMenuLabelControl', CenterPos, ControlOffset, LabelWidth, 1));
	SpecTPLabel.SetText("Teleport 1500 units forward");
	SpecTPLabel.SetTextColor(Basecolor);
	SpecTPLabel.SetFont(F_Normal);

	SpecTPButton = UMenuRaisedButton(CreateControl(class'UMenuRaisedButton', ButtonLeft, ControlOffset-3, ButtonWidth, 1));
	SpecTPButton.bAcceptsFocus = False;
	SpecTPButton.bIgnoreLDoubleClick = True;
	SpecTPButton.bIgnoreMDoubleClick = True;
	SpecTPButton.bIgnoreRDoubleClick = True;

	ResetSpecTPButton = UWindowSmallButton(CreateControl(class'UWindowSmallButton', ResetLeft, ControlOffset-2, ButtonWidth, 20));
	ResetSpecTPButton.DownSound = Sound'Botpack.Click';
	ResetSpecTPButton.Text = "Remove";
...
function ProcessMenuKey(int KeyNo, string KeyName)
{
	...
	else if(SpecTPButton.bDisabled)
	{
		GetPlayerOwner().ConsoleCommand("SET INPUT " $ SpecTPButton.Text);
		GetPlayerOwner().ConsoleCommand("SET INPUT " $ KeyName $ " SPECTP");
		SelectedButton.SetText(KeyName);
	}
}
			...
			case DE_Click:
			    ...
			    switch(C)
        	    {
					case ResetBehindviewToggleButton:
						...
						LoadExistingKeys();
					break;
					case ResetSpecTPButton:
						GetPlayerOwner().ConsoleCommand("SET INPUT " $ SpecTPButton.Text);
						SpecTPButton.SetText("");
						LoadExistingKeys();
					break;
